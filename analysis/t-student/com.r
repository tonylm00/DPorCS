...
data <- data.frame(
  ...
  com = c(d1$A, d2$B, d3$C, d4$D)
)

# Confronto dei quattro gruppi utilizzando il test del t di Student
# Rispetto al Gruppo A
t_test_result <- pairwise.t.test(data$com, data$Gruppo, p.adjust.method = "none")
t_test_result
...
bonferroni_adjusted_p <- pairwise.t.test(data$com, data$Gruppo, p.adjust.method = "bonferroni")
